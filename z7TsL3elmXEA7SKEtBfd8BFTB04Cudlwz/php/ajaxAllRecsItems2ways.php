...
//$thisFileName = "ajaxAllRecsItems2ways.php";        ######## USED BY ALL COLUMNS EXCEPT Withdrawn/Paidin AND DIRECT EDITING ONES - Reference/Note ########
...
//NOW USES CONSTANT FROM index.php $columnNameAry = array("recordDate", "personOrOrg", "transCatgry", "", "", "accWorkedOn", "budget", "referenceInfo", "reconciledDate", "umbrella", "docType", "recordNotes");
$columnName = $_fieldNameAry[$columnId]; //create columnName from column id (0 - 11). The array would need to be changed if the column order (and therefore the cell ids) on the display page changes!!

$strInsteadOfKey = FALSE;
switch ($columnName) { //choose the appropriate item list for the field that is being updated.
  case "personOrOrg":
...
if ($strInsteadOfKey) { //direct string write to table without converting to key
    ...
    try {
        $stmt = $conn->prepare('UPDATE allRecords SET '.$columnName.'=? WHERE idR = ?');
        $stmt->execute(array($str, $rowId));

        $stmt = $conn->prepare('SELECT '.$columnName.' FROM allRecords WHERE idR = ?');
        $stmt->execute(array($rowId));
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $updatedFieldStr = $row[$columnName];
    ...
    try {
        if ($allowedToEdit || subAryContains($restrictionsAry, "allowColumnEdit", $columnName)) { //only allow changes to table if $allowedToEdit is true OR the subarry of $restrictionsAry selected by "allowColumnEdit" contains $columnName (this allows granular setting of edit privilages to be applied to chosen columns when a general - default condition - no editing directive has been enforced)
          $stmt = $conn->prepare('UPDATE allRecords SET '.$columnName.'=? WHERE idR = ?');
